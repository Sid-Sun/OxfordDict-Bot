...
func (m *MSSQLRedisStore) InsertUser(ChatID int64) {
	query := `INSERT INTO users (user_id, notified, query_count) VALUES (@userID, @notified, @queryCount)`

	_, err := m.sql.Exec(query, sql.Named("userID", ChatID), sql.Named("notified", true), sql.Named("queryCount", 1))
	if err != nil {
